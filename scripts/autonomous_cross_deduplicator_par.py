...
def assemble_targets(input_dir: Path, output_dir: Path,
    ...
    """
    full_list = [dir.name for dir in sorted(input_dir.iterdir())
                 if has_minhash_content(dir)]
    if upto_dir:
        try:
            upto_i = full_list.index(upto_dir)
        except ValueError:
            logging.error('The upto-dir is not a valid batch')
            raise
        full_list = full_list[: upto_i+1]
    if from_dir:
        try:
            from_i = full_list.index(from_dir)
        except ValueError:
            logging.error('The from-dir is not a valid batch')
            raise
        target_list = full_list[from_i:]
    else:
        from_i = 0
        target_list = full_list
    logging.debug(f'The list of targets is: {target_list} \n They start from '
                  f'pos {from_i} of the relevant history: {full_list}')
    pairings = []
